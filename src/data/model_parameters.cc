...
// Initialize model from a checkpoint file
Model::Model(const std::string &model_filename, const std::string &white_list_filename)
{
  CHECK_NE(model_filename.empty(), true);
  CHECK_NE(white_list_filename.empty(), true);

  if (this->Deserialize(model_filename) == false)
  {
    Color::print_error(
        StringPrintf("Cannot Load model from the file: %s",
  ...
  if (this->LoadWhiteList(white_list_filename) == false)
  {
    Color::print_error(
        StringPrintf("Cannot Load white list from the file: %s",
                     white_list_filename.c_str()));
    exit(0);
  }
}
...
bool Model::LoadWhiteList(const std::string &filename)
{
  for (index_t i = 0; i < num_field_; ++i)
  {
    white_list_[{-1, i}] = false;

    // j ranges from i to num_field_ to include the field X itself.
    for (index_t j = i; j < num_field_; ++j)
    {
      white_list_[{i, j}] = false;
    }
  }

  std::ifstream in(filename);
  index_t field_1, field_2;

  while (in >> field_1 >> field_2)
  {
    if (field_1 > field_2)
    {
      std::swap(field_1, field_2);
    }

    if (white_list_.find({field_1, field_2}) == white_list_.end())
    {
      return false;
    }
    else
    {
      white_list_[{field_1, field_2}] = true;
    }
  }
...
//
bool Model::is_white(index_t field_1, index_t field_2)
{
  if (field_1 > field_2)
  {
    std::swap(field_1, field_2);
  ...
  {
    Color::print_error(
        StringPrintf("The field %d X field %d is invalid", field_1, field_2));
    exit(0);
  }
  else
  {
    return white_list_[{field_1, field_2}];
  }
}
