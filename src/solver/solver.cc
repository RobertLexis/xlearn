...
void Solver::Initialize(int argc, char *argv[])
{
  ...
  else if (hyper_param_.is_embedding)
  {
    init_embedding();
  }
  else
...
void Solver::init_train()
{
  ...
  { // Initialize parameter from pre-trained model
...
void Solver::init_hyper_param_from_model()
{
...
void Solver::init_embedding()
{
  ...
  pool_ = new ThreadPool(threadNumber);
  Color::print_info(
      StringPrintf("xLearn uses %i threads for embedding task.",
                   threadNumber));
  /*********************************************************
   *  Read model file                                      *
   *********************************************************/
  ...
  Timer timer;
  timer.tic();
  model_ = new Model(hyper_param_.model_file, hyper_param_.white_list_file);

  init_hyper_param_from_model();
...
{
  Color::print_action("Start to embedding ...");

  Embedder embedder;
  embedder.Initialize(reader_[0],
                      model_,
                      hyper_param_.output_file,
                      pool_);
  // Predict and write output
